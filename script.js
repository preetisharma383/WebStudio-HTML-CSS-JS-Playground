...
window.addEventListener('DOMContentLoaded', () => {
  ...
  function initWhenAceReady() {
    if (typeof ace === 'undefined') {
      log('Waiting for ACE editor to load...', 'warn');
      setTimeout(initWhenAceReady, 200); // retry until ace loads
      return;
    }
...
<\/script>
    ...
    function loadProject(raw){
      ...
      ed_html.setValue(proj.html, -1);
      ed_css.setValue(proj.css, -1);
      ed_js.setValue(proj.js, -1);
      log('Project loaded.');
    }
...
console.log('Hello from JavaScript!');`, -1);
  ...
  initWhenAceReady(); // start initialization once DOM is ready
});
